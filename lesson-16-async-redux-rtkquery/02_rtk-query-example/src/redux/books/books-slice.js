...
import { fetchBooks, addBook, removeBook, updateFavoriteStatus } from "./books-operation";
...
const booksSlice = createSlice({
    ...
    extraReducers: {
        ...
        [updateFavoriteStatus.pending]: pendingCallback,
        [updateFavoriteStatus.fulfilled]: (store, {payload}) => {
            store.loading = false;
            const index = store.items.findIndex((item) => item.id === payload.id);
            store.items[index] = payload;
        },
        [updateFavoriteStatus.rejected]: rejectedCallback,
    }
})
